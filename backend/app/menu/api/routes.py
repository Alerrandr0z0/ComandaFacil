...
from app.dependencies import DbSession, MongoDB
...
async def create_menu(
    ...
    command = CreateMenuCommand(id=schema.id, name=schema.name, description=schema.description)
...
async def list_menus(db: DbSession) -> list[MenuResponseSchema]:
    repo = SQLAlchemyMenuRepository(db)
    handler = ListMenusHandler(repo)
    menus = await handler.handle(ListMenusQuery())
...
async def get_menu(menu_id: int, db: DbSession) -> MenuResponseSchema:
    repo = SQLAlchemyMenuRepository(db)
    handler = GetMenuHandler(repo)
    menu = await handler.handle(GetMenuQuery(menu_id=menu_id))
    if not menu:
...
async def add_menu_item(
    ...
    menu = await repo.find_by_id(menu_id)
    if menu:
        background_tasks.add_task(MenuReadModelSync(mongo).sync, menu)
...
async def remove_menu_item(
    ...
    command = RemoveMenuItemCommand(menu_id=menu_id, item_id=item_id)
...
async def toggle_menu(
    ...
    command = ToggleMenuCommand(menu_id=menu_id, activate=schema.activate)
    menu = await handler.handle(command)
    await db.commit()
...
async def delete_menu(
    ...
    command = DeleteMenuCommand(menu_id=menu_id)
    await handler.handle(command)
    await db.commit()
